...
$.getJSON("ruabien_bat.geojson", function (ruabat) {
    $.getJSON("forest.geojson", function (forest) {
        //console.log(data);
        /*--- Search Bar ---*/
        ...
        /*---Phải tạo LayerGroup---*/
        var rua_bat = L.layerGroup();
        /*---Tạo Chart---*/
        var charts = {};
        for (var attr in ruabat.features) {
            //console.log(ruabat.features[attr].properties)
            var props = ruabat.features[attr].properties;
            var data_feat = [
                props['2015'],
                props['2016'],
                props['2017'],
            ...
            charts[props['NL_NAME_3']].bindPopup("<div id='popup_tb'>" + "<table>" +
                ...
                "</tbody>" +
                "</table>" + "</div>");
            rua_bat.addLayer(charts[props['NL_NAME_3']]);
        }
        ...
        var map = L.map('mymap', {
                ...
                zoom: 7,
                layers: [op_street, base, rua_bat, forest_tn],
                zoomControl: false
            }
        );
        ...
        var ruabat_legend = L.control({position: 'bottomright'});
        ruabat_legend.onAdd = map => {
            var div = L.DomUtil.create('div', 'info legend');
        ...
        ruabat_legend.addTo(map);

        map.on("overlayadd", function (e) {
            if (e.name == "Rùa biển") {
                ruabat_legend.addTo(this);
            } else if (e.name == "Rừng") {
        ...
        map.on("overlayremove", function (e) {
            if (e.name == "Rùa biển") {
                map.removeControl(ruabat_legend);
            } else if (e.name == "Rừng") {
